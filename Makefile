...
compile_solidity_multi:
	@echo "compiling test/multi solidity sources to .abi and .bin files"
	@solc --evm-version paris -o ./test/multi --abi --bin --overwrite ./test/multi/Multi.sol

compile_go_multi:
	@echo "compiling test/multi .abi and .bin files to go bindings, writing to pkg/testing/unit/multi"
	@abigen --abi ./test/multi/Multi.abi --bin ./test/multi/Multi.bin -pkg multi -type Multi -out ./pkg/testing/unit/multi/multi.go

compile_assert: compile_solidity_assert compile_go_assert
compile_erc20: compile_solidity_erc20 compile_go_erc20
compile_multi: compile_solidity_multi compile_go_multi
...
all: clean_test compile_assert compile_erc20 compile_multi
